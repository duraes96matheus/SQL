use master 
drop database aula04042018
create database aula04042018
use aula04042018
...
select *from tb_clientes	--exibi toda a tabela
select *from tb_hardware	--exibi toda a tabela
select *from tb_vendas		--exibi toda a tabela

select nome, fone  from tb_clientes		--exibi toda a tabela

select preco from tb_hardware			--exibi toda a tabela

select preco from tb_hardware order by preco asc		 --exibi toda a tabela de baixo pra cima

select preco from tb_hardware order by preco desc		 --exibi toda a tabela de cima pra baixo

select nome from tb_clientes order by nome asc				--exibi toda a tabela de baixo pra cima
select preco from tb_hardware order by preco desc			--exibi toda a tabela de cima pra baixo

select nome from tb_clientes order by nome desc  --			--exibi toda a tabela de cima pra baixo

select nome from tb_clientes where nome='matheus'  -- encontra um valor na tabela

select fone from tb_clientes where fone='20584560'	-- encontra um valor na tabela

select preco from tb_hardware where preco ='260'	 -- encontra um valor na tabela

select nome from tb_clientes where nome like 'ma%'   --exibe os atributos que tem esta particula

select preco from tb_hardware order by preco asc

select descricao,preco from tb_hardware order by preco asc
